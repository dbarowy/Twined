...
\subsection*{Remaining Work} 
  \begin{enumerate}
    \item
      \begin{spacing}{1.2}
        \hspace*{1cm} 
        Give the user the capibility to supply a PDF and other input to be summarized 
        and have a graph created of, we nearly have the PDF functionality but werent able to get it functioning 
        by the deadline while also getting a working web interface. 
      \end{spacing}
    \item
      \begin{spacing}{1.2}
        \hspace*{1cm} 
        Improve the parser to allow more flexible inputs to generate graphs. This came up once we got to the point
        of integrating chatGPT. Sometimes chatGPT would forget a "," or some other punctuation that would derail everything,
        it would be nice fix this to prevent graph display errrors.
      \end{spacing}
    \item
      \begin{spacing}{1.2}
        \hspace*{1cm} 
        The web interface is curently a bit clunky. That is probably to be expected considering the differance in resources between
        us as a company like Google or Meta. But it would be nice to get to the point where any input can be accepted bu the chat box
